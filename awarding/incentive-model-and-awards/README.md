...
In order to ensure fairness and to incentivize prompt reporting of such issues, findings meeting these criteria will be the only ones in the duplicate set eligible for payout from the competitive audit pool. Duplicates of such findings that are not submitted via the proper procedure will not be eligible for awards.
